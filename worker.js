...
function calculateLevenshteinAndDiff(s1, s2) {
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && s1[i - 1] === s2[j - 1]) {
      ...
      i--;
      j--;
    } else if (j > 0 && (i === 0 || dp[getIndex(i, j)] === dp[getIndex(i, j - 1)] + 1)) {
      html2 = `<span class="diff-added">${escapeHtml(s2[j - 1])}</span>` + html2;
      j--;
    } else if (i > 0 && (j === 0 || dp[getIndex(i, j)] === dp[getIndex(i - 1, j)] + 1)) {
      html1 = `<span class="diff-removed">${escapeHtml(s1[i - 1])}</span>` + html1;
      i--;
    } else if (i > 0 && j > 0 && dp[getIndex(i, j)] === dp[getIndex(i - 1, j - 1)] + 2) {
      html1 = `<span class="diff-removed">${escapeHtml(s1[i - 1])}</span>` + html1;
      html2 = `<span class="diff-added">${escapeHtml(s2[j - 1])}</span>` + html2;
      i--;
      j--;
    } else {
      console.warn("Unexpected path in diff reconstruction. Falling back to default step.");
      if (i > 0) i--;
      else if (j > 0) j--;
    }
  }
